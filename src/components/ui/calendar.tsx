        ...
        Dropdown: (props) => {
          ...
          if (props.name === "months") {
            ...
            return (<Select onValueChange={(newValue) => {
              ...
              <SelectTrigger>{format(currentMonth, "MMM", { locale: es }).toUpperCase()}</SelectTrigger>
              <SelectContent>
                {selectItems.map((selectItem) => (
                  <SelectItem key={selectItem.value} value={selectItem.value}>
                    {selectItem.label ? selectItem.label.charAt(0).toUpperCase() + selectItem.label.slice(1) : selectItem.label}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>)
