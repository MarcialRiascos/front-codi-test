...
}).refine((data) => data.Telefono || data.Celular || data.TelefonoTres, { message: "Al menos un número de teléfono es obligatorio" });
export const beneficiaryDocumentSchema = z.object({
    contrato: z.object({
        file: z.custom()
            .refine((file) => !file || (file instanceof File && file.type.startsWith("application/pdf")), "El archivo debe ser de tipo pdf")
            .refine((file) => !file || file.size < 1024 * 1024 * 5, "El archivo debe ser menor a 5MB").optional(),
    }),
    copia_cc: z.object({
        file: z.custom()
            .refine((file) => !file || (file instanceof File && file.type.startsWith("application/pdf")), "El archivo debe ser de tipo pdf")
    ...
    foto_fp: z.object({
        file: z.custom()
            .refine((file) => !file || (file instanceof File && file.type.startsWith("image/")), "El archivo debe ser de tipo imagen")
            .refine((file) => !file || file.size < 1024 * 1024 * 5, "El archivo debe ser menor a 5MB").optional()
    }),
    ...
    recibo: z.object({
        file: z.custom()
            .refine((file) => !file || (file instanceof File && file.type.startsWith("application/pdf")), "El archivo debe ser de tipo imagen")
            .refine((file) => !file || file.size < 1024 * 1024 * 5, "El archivo debe ser menor a 5MB").optional()
    }),
    ...
    documents: z.array(z.object({
        file: z
            ...
            if (!ACCEPTED_FILE_TYPES.includes(file.type)) {
                ctx.addIssue({
                    code: z.ZodIssueCode.custom,
                    message: "Tipos de archivos permitidos: PNG, JPG, JPEG, PDF, DOC, DOCX",
                });
            }
